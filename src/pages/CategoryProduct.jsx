...
const CategoryProduct = () => {
  ...
  const category = params.category;
  const navigate = useNavigate();
  const { data } = getData(); // ✅ Using context data instead of API

  useEffect(() => {
    if (data && data.length > 0) {
      // ✅ Filter locally from context
      ...
      <div className="">
        {searchData.length > 0 ? (
          <div className="max-w-6xl mx-auto mt-10 mb-10 px-4">
            <button
              onClick={() => navigate("/")}
              className="bg-gray-800 text-white mb-5 px-3 py-1 rounded-md cursor-pointer flex gap-1 items-center"
            >
              <ChevronLeft /> Back
            </button>
